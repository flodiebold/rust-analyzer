...
impl<'db> DbInterner<'db> {
    pub(crate) fn new() -> DbInterner<'db> {
    ...
    pub(crate) fn db(&self) -> &'db dyn HirDatabase {
        // SAFETY: ¯\_(ツ)_/¯
...
impl<'db> rustc_type_ir::Interner for DbInterner<'db> {
    ...
    fn associated_type_def_ids(self, def_id: Self::DefId) -> impl IntoIterator<Item = Self::DefId> {
        ...
        let associated_ty_ids: Vec<_> =
            self.db().trait_items(trait_).associated_types().map(|id| id.into()).collect();
        associated_ty_ids
    }
