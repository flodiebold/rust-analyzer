...
use crate::next_solver::{ParamEnv, Ty};
type Span = ();
use smallvec::SmallVec;

use self::abs_domain::{AbstractElem, Lift};
use super::un_derefer::UnDerefer;
...
pub struct MoveData<'tcx> {
    ...
    pub moves: IndexVec<MoveOutIndex, MoveOut<'tcx>>,
    ...
    pub loc_map: LocationMap<'tcx, SmallVec<[MoveOutIndex; 4]>>,
    pub path_map: IndexVec<MovePathIndex, SmallVec<[MoveOutIndex; 4]>>,
    pub rev_lookup: MovePathLookup<'tcx>,
    pub inits: IndexVec<InitIndex, Init<'tcx>>,
    /// Each Location `l` is mapped to the Inits that are effects
    /// of executing the code at `l`.
    pub init_loc_map: LocationMap<'tcx, SmallVec<[InitIndex; 4]>>,
...
#[derive(Debug)]
pub struct LocationMap<'db, T> {
    /// Location-indexed (BasicBlock for outer index, index within BB
    /// for inner index) map.
    pub(crate) map: ArenaMap<BasicBlock<'db>, Vec<T>>,
}

impl<'db, T> Index<Location<'db>> for LocationMap<'db, T> {
    type Output = T;
    fn index(&self, index: Location<'db>) -> &Self::Output {
        &self.map[index.block][index.statement_index]
    }
}

impl<'db, T> IndexMut<Location<'db>> for LocationMap<'db, T> {
    fn index_mut(&mut self, index: Location<'db>) -> &mut Self::Output {
        &mut self.map[index.block][index.statement_index]
    }
}

impl<'db, T> LocationMap<'db, T>
where
    T: Default + Clone,
{
    fn new(body: &Body<'_>) -> Self {
        LocationMap {
            map: body
                .basic_blocks
                .iter()
                .map(|(id, block)| (id, vec![T::default(); block.statements.len() + 1]))
                .collect(),
        }
    }
}
...
#[derive(Copy, Clone)]
pub struct MoveOut<'db> {
    /// path being moved
    pub path: MovePathIndex,
    /// location of move
    pub source: Location<'db>,
}

impl<'db> fmt::Debug for MoveOut<'db> {
...
pub enum InitLocation<'db> {
    Argument(Local<'db>),
    Statement(Location<'db>),
}
...
pub struct MovePathLookup<'tcx> {
    locals: ArenaMap<Local<'tcx>, Option<MovePathIndex>>,
...
impl<'tcx> MovePathLookup<'tcx> {
    ...
    pub fn iter_locals_enumerated(
        ...
        self.locals.iter().filter_map(|(l, &idx)| Some((l, idx?)))
    }
}
