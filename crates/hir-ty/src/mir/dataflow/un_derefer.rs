/// Ported with minor changes from `rustc_mir_dataflow`!

use rustc_hash::FxHashMap;
use crate::mir::*;
type Local<'db> = crate::mir::LocalId<'db>;


/// Used for reverting changes made by `DerefSeparator`
#[derive(Default, Debug)]
pub(crate) struct UnDerefer<'tcx> {
    deref_chains: FxHashMap<Local<'tcx>, Vec<PlaceRef<'tcx>>>,
}

impl<'tcx> UnDerefer<'tcx> {
    #[inline]
    pub(crate) fn insert(&mut self, local: Local<'tcx>, reffed: PlaceRef<'tcx>) {
    ...
    #[inline]
    pub(crate) fn deref_chain(&self, local: Local<'tcx>) -> &[PlaceRef<'tcx>] {
