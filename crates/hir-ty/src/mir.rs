...
use rustc_type_ir::{TyKind, inherent::{GenericArgs as _, IntoKind as _, SliceLike as _, Ty as _}};
...
impl<'db> Place<'db> {
    ...
    pub fn ty<D: ?Sized>(&self, body: &MirBody<'db>, tcx: DbInterner<'db>) -> PlaceTy<'db>
    {
...
impl<'tcx> PlaceTy<'tcx> {
    ...
    pub fn projection_ty<V: ::std::fmt::Debug>(
        ...
        self.projection_ty_core(tcx, &elem, |ty| ty, |self_ty, _variant, field_id| {
            match self_ty.kind() {
                TyKind::Adt(adt_def, args) if !adt_def.is_enum() => {
                    tcx.db().field_types(adt_def.non_enum_variant().id())[field_id.unwrap_left()].instantiate(tcx, args)
                }
                TyKind::Tuple(tys) => tys
